...
while running:
    ...
    for row in range(GRID_WIDTH):
        for col in range(GRID_HEIGHT):
            ...
            if grid_unit == I:
                ...
                pellet = True
                pygame.draw.circle(
                    screen, WHITE, (cell_x + GRID_SIZE // 2, cell_y + GRID_SIZE // 2), 5
                )
            elif grid_unit == O:
                pygame.draw.circle(
                    screen,
                    WHITE,
                    (cell_x + GRID_SIZE // 2, cell_y + GRID_SIZE // 2),
                    10,
                )
            elif grid_unit == c:
                pygame.draw.circle(
                    screen, RED, (cell_x + GRID_SIZE // 2, cell_y + GRID_SIZE // 2), 5
                )
    ...
    pacman_rect = pygame.Rect(
        pacman_x * GRID_SIZE, pacman_y * GRID_SIZE, GRID_SIZE, GRID_SIZE
    )
    pygame.draw.circle(
        screen,
        YELLOW,
    ...
    mouth_angle = 40
    for angle in range(
        orientation_angle - mouth_angle, orientation_angle + mouth_angle + 1
    ):
        x = (
            pacman_x * GRID_SIZE
            + GRID_SIZE // 2
            + pacman_radius * math.cos(math.radians(angle))
        )
        y = (
            pacman_y * GRID_SIZE
            + GRID_SIZE // 2
            + pacman_radius * math.sin(math.radians(angle))
        )
